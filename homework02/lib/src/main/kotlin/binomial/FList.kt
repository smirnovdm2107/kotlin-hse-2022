...
sealed class FList<T> : Iterable<T> {
    ...
    data class Cons<T>(val head: T, val tail: FList<T>) : FList<T>() {
        ...
        private tailrec fun filter(list: FList<T>, iterator: Iterator<T>, f: (T) -> Boolean): FList<T> {
            if (!iterator.hasNext()) {
                return list
            }
        ...
        override fun <U> fold(base: U, f: (U, T) -> U): U = fold(this.iterator(), base, f)

        private tailrec fun <U> fold(iterator: Iterator<T>, base: U, f: (U, T) -> U): U {
            if (!iterator.hasNext()) {
                return base
            }
            val nextBase = f(base, iterator.next())
            return fold(iterator, nextBase, f)
        }
    }
